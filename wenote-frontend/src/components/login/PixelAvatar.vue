<template>
  <div class="flex justify-center mb-6 relative z-30">
    ...
    <Transition name="bubble">
      <div v-if="message" class="absolute -top-20 bg-white border-4 border-black p-3 rounded-2xl shadow-lg w-48 text-center">
        <p class="text-xs font-bold text-slate-800 leading-tight">{{ message }}</p>
    ...
    <div
      ref="snakeRef"
      @click="$emit('click')"
      class="w-32 h-28 rounded-[50%] border-4 border-black relative flex items-center justify-center shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] cursor-pointer transition-all duration-300 hover:scale-105"
      :class="combo > 5 ? 'bg-emerald-400' : 'bg-green-500'"
    >
      <!-- 皇冠 -->
      <Transition name="crown">
        <svg v-if="combo > 5" class="absolute -top-10 w-10 h-10 text-yellow-400 drop-shadow-[2px_2px_0px_rgba(0,0,0,1)] animate-bounce" fill="currentColor" viewBox="0 0 24 24">
      ...
      <div class="flex gap-6 mb-2">
        <!-- 左眼 -->
        <div class="w-8 h-8 bg-white rounded-full border-2 border-black flex items-center justify-center overflow-hidden">
          ...
          <template v-else>
            <!-- 睁眼 - 眼球跟随鼠标 -->
            <div
              class="w-4 h-4 bg-black rounded-full transition-transform duration-100"
      ...
      <!-- 腮红 -->
      <div class="absolute bottom-6 left-4 w-4 h-2 bg-pink-300 rounded-full opacity-60"></div>
      <div class="absolute bottom-6 right-4 w-4 h-2 bg-pink-300 rounded-full opacity-60"></div>
