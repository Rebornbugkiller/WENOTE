<template>
  <div class="daily-goal bg-white border-4 border-black rounded-2xl p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
    <div class="flex justify-between items-center mb-2">
      <div class="text-xs font-black text-slate-400 uppercase tracking-wider">每日目标</div>
    ...
    <div class="flex items-end gap-2 mb-3">
      ...
      <span class="text-slate-400 font-bold text-sm pb-1">/ {{ target.toLocaleString() }} 字</span>
    </div>
    ...
    <div v-if="completed" class="mt-3 text-center">
      ...
      <span class="text-green-600 font-black text-sm ml-2">目标达成！</span>
    </div>
      ...
      <Transition name="modal">
        <div v-if="showSettings" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm" @click.self="showSettings = false">
          <div class="bg-white border-4 border-black rounded-2xl p-6 w-[90vw] max-w-sm shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            <h3 class="text-xl font-black mb-4">设置每日目标</h3>
            <div class="mb-4">
              <label class="block text-sm font-bold text-slate-600 mb-2">每日字符目标</label>
              <input
              ...
              />
              <p class="text-xs text-slate-400 mt-1">范围: 100 - 10000 字</p>
            </div>
