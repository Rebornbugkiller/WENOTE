    ...
    <div>
      ...
      <div class="grid grid-cols-8 gap-2">
        <button v-for="[key, emoji] in avatarList" :key="key"
                ...
                class="w-12 h-12 rounded-xl border-4 text-2xl flex items-center justify-center transition-all hover:scale-110"
                :class="style === key
    ...
    <div>
      ...
      <div class="flex gap-3">
        <button v-for="c in AVATAR_COLORS" :key="c"
                ...
                class="w-10 h-10 rounded-xl border-4 transition-all hover:scale-110"
                :class="color === c
